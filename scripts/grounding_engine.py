#!/usr/bin/env python3
"""
grounding_engine.py — the importable citation-integrity engine.
...
the code at the cited line. A deterministic engine can't, and faking that would
...
This module is pure logic with no stdin/stdout/sys.exit — the hook entrypoint
(grounding-verifier.py) is a thin shim over run(). The forced-continuation
budget lives in loop_guard.py. Both are imported here.
...
# Single source of truth for the tool taxonomy. Both this engine and the
...
def _check_bash_atom(atom_text, bash_calls):
    ...
    the engine renders that output, the model never transcribes it. Backtick
...
def _render_output(n_runs, output, max_lines=3, max_chars=200):
    """An indented block showing recorded Bash output in the engine's own
    report. The engine is the SOURCE of this text (not the model), so trimming
...
def build_response(event, findings, stats, cited, block, note):
    """The single seam where the two host adapters live: given a hook event and
    the verify/guard verdict, return the JSON object to print — or None when
    there is nothing to say. The block path's envelope differs by host (Stop's
    {"decision":"block"} vs PreToolUse's permissionDecision:deny); the warn path
    is one systemMessage for both. Pure — no I/O — so it is tested directly."""
    if block:
        reason = (
    ...
    # Warn path: emit a report if there are findings OR there's a non-empty trust
    # summary (so a clean answer with citations still gets the positive tier line,
    # and silence unambiguously means "no citations to report on").
    if not (findings or summary_line(stats)):
        return None
    msg = report(findings, stats, cited)
    if note:  # we WOULD have blocked but a guard declined — say so for the human
        msg += f"\n(not blocking: {note})"
    return {"systemMessage": msg}


def run(data):
    """Orchestrate one hook invocation: the parsed hook-input dict in, the
    response dict to print (or None) out. No stdin/stdout/sys.exit — the
    entrypoint owns those, so this stays importable and directly testable.

    The verifier is wired to BOTH Stop (turn end) and PreToolUse/AskUserQuestion
    (Claude is asking the user a question — Stop does NOT fire at that pause);
    build_response selects the right output envelope from data['hook_event_name'].
    """
    ...
    block, note = loop_guard.should_block(session_id, blocking)
    return build_response(event, findings, stats, cited, block, note)
